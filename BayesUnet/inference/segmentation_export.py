...
def save_segmentation_nifti_from_softmax(bayesian: bool,
    ...
    :return:
    """
    print("Bayesian in segm_exp:", bayesian)

    if verbose: print("force_separate_z:", force_separate_z, "interpolation order:", order)

    if isinstance(segmentation_softmax, str): # #MŚ: if it is a filename
    ...
    #### NASZA ZMIANA ####
    if not bayesian:
      print("Categorize traditionally")
      ...
      print("Predict uncertainty for array with shape ",seg_old_spacing.shape)
      seg_old_spacing = seg_old_spacing[2] # MŚ: liczba rzeczywista, bez zaokrąglenia w klasyfikacji - niepewność
    
    # MŚ: check if the values are real or binary:
    print("Type and size of seg_old_spacing: ", type(seg_old_spacing), type(seg_old_spacing[0, 0, 0]), seg_old_spacing.shape)
    print("Example element of seg_old_spacing: ", seg_old_spacing[0, 0, 0])
    ####
    ...
    ###### MŚ: save float values instead of int
    if not bayesian:
        print("Saving type: ", type(seg_old_size_postprocessed.astype(np.uint8)[0, 0, 0]))
        seg_resized_itk = sitk.GetImageFromArray(seg_old_size_postprocessed.astype(np.uint8))
    else:
        print("Saving type: ", type(seg_old_size_postprocessed[0, 0, 0]))
        seg_resized_itk = sitk.GetImageFromArray(seg_old_size_postprocessed) #.astype(np.uint8))
    ######
    ...
    if (non_postprocessed_fname is not None) and (seg_postprogess_fn is not None):
        ###### MŚ: save float values instead of int
        if not bayesian:
            print("Saving type: ", type(seg_old_size.astype(np.uint8)[0, 0, 0]))
            seg_resized_itk = sitk.GetImageFromArray(seg_old_size.astype(np.uint8))
        else:
            print("Saving type: ", type(seg_old_size[0, 0, 0]))
            seg_resized_itk = sitk.GetImageFromArray(seg_old_size) #.astype(np.uint8))
        ######
