...
def run_iteration(tr_gen,network,optimizer,loss,amp_grad_scaler,fp16=True,do_backprop=True):
    ...
    if fp16:
        with autocast():
            outputA, outputB = network(data)
            del data
            l = loss(outputA, target) + loss(outputB, target)
...
network = Model(in_ch=3, out_ch=1,list_ch_A=[-1, 16, 32, 64, 128, 256],list_ch_B=[-1, 32, 64, 128, 256, 512])


summary(network, input_size=(1, 3, 64, 64,64))
