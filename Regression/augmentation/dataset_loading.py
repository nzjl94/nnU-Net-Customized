...
from batchgenerators.utilities.file_and_folder_operations import listdir, isfile, subfiles, join, load_pickle


def get_case_identifiers(folder):
    case_identifiers = [i[:-4] for i in listdir(folder) if i.endswith("npz") and (i.find("segFromPrevStage") == -1)]
    return case_identifiers


def get_case_identifiers_from_raw_folder(folder):
    case_identifiers = np.unique(
        [i[:-12] for i in listdir(folder) if i.endswith(".nii.gz") and (i.find("segFromPrevStage") == -1)])
    return case_identifiers
...
def load_dataset(folder, num_cases_properties_loading_threshold=1000):
    """
    Loads data ("we don't load the actual data but instead return the filename to the np file.") from the specified
     in config_loading.py file location.
    :param folder: e. g. folder_with_preprocessed_data = "nnUNet_preprocessed/nnUNetData_plans_v2.1_stage0"
    :param num_cases_properties_loading_threshold: the number of loaded cases will be less than or equal to
    num_cases_properties_loading_threshold number.
    ...
    #print('\n*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*\n')
    #print('dataset items: {}'.format(len(dataset.items())))
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
        ...
        #print('###########################')
        #print('my dataloader')
        #print('\t\t',self.patch_size, self.final_patch_size, self.batch_size)
        #print('###########################')
